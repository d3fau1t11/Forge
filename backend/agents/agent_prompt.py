...
class AgentContext:
    ...
    # Strategy exhaustion (swarm-enforced) ──────────────────────────────────
    # Strategies already tried by the whole swarm without producing new evidence;
    # injected into the prompt as a mandatory ban.  Populated from
    # board.exhausted_strategies by _build_agent_context().
    exhausted_strategies: list = field(default_factory=list)
    # Free-text pivot instruction generated by strategic_planner.review_swarm_pivot()
    # when a strategy is marked exhausted.  Shared across all agents.
    pivot_directive: str = ""
...
7. Begin EVERY response with a line: STRATEGY: <short_snake_case_label>
   This declares the *class* of approach for this turn, separate from the specific command.
   Examples: credential_stuffing, hash_offline_crack, session_forgery, source_review,
   sql_injection, idor_bypass, jwt_tamper, cookie_forge, path_traversal, ssrf_probe.
   The swarm coordinator reads this label BEFORE executing your command; if the swarm has
   already exhausted this class of approach across all agents, your command will be blocked
   and you will receive a mandatory pivot directive instead."""
...
def build_user_prompt(ctx: AgentContext) -> str:
    ...
    # Strategy ban section (swarm-enforced exhaustion + forced pivot)
    if ctx.exhausted_strategies or ctx.pivot_directive:
        ban_lines = ["=== STRATEGY GATE (SWARM-ENFORCED — MANDATORY) ==="]
        if ctx.exhausted_strategies:
            ban_lines.append(
                "The following approaches are EXHAUSTED across all swarm agents and "
                "MUST NOT be repeated: " + ", ".join(sorted(ctx.exhausted_strategies)) + "."
            )
        if ctx.pivot_directive:
            ban_lines.append(f"Required pivot: {ctx.pivot_directive}")
        ban_lines.append(
            "Choose a strategy label NOT in the exhausted list above. "
            "If your intended approach is exhausted, pick a genuinely different class."
        )
        strategy_ban_section = "\n".join(ban_lines)
    else:
        strategy_ban_section = ""
    ...
    return USER_PROMPT_TEMPLATE.format(
        ...
        strategy_ban_section=strategy_ban_section,
    )
...
def make_context_from_env(
    ...
    injected_directive: str = "",
    memory_context: str = "",
    exhausted_strategies: list | None = None,
    pivot_directive: str = "",
) -> AgentContext:
    ...
    return AgentContext(
        ...
        exhausted_strategies=list(exhausted_strategies or []),
        pivot_directive=pivot_directive or "",
    )
